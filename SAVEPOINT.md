...
## 6. 開発状況と次ステップ
- **[Why]**: アプリ操作休止時のGAS通信対策および不意の再読込に対する耐障害性の向上。
- **[How]**: ユーザーの細かな入力（リアクティブ操作）を除外し、Tab2の「主要な確定動作時」に限定して状態を退避・復元することで、既存フローを汚染せずUXを劇的に改善する。
  - 1: **アイドル超過トースト**: Class IdleTimerを新設し未操作時間を検知。一定時間超過でHTML内カスタム永続トーストを介し再読込を誘導。
  - 2: **状態バックアップ**: Tab2の主要な非同期処理終了（CoreCtrl.withAsyncLock）時に、JSON化したstateをGASプロパティへ非同期保存。リアクティブ操作は対象外。
  - 3: **整合性検証（軽量化と安全網）**: 巨大な生データ（PXRF, WDXRF, Correction）は対象外とし、シート側の実データと保存されたstateの整合性を起動時に走査。
  - 4: **復元機構の駆動**: 再起動時（Evt.init）に外部バックアップの有無を確認し、3の整合性が取れた場合のみ自動復元。不一致時は安全に初期状態へフォールバック。
- **[do]**: 上記使用について以下の項目を評価し、ユーザーとの対話により論理構成をすり合わせる
  - 2の発火タイミングとUX阻害の可能性
  - stateに触れることによる処理フロー汚染の可能性と安定性
- **[Rule]**: 既存のUI挙動の崩壊につながる改修は絶対禁止。state連動するCSSについては依存関係を整理。より最適な別アプローチを提案できるならば0ベースの提案も許可する。
