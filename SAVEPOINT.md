...
## 6. 次の開発手順
- CSSのリファクタリング
- CSSの整理案: 全体をセクション分けし、CSS.html 全体を「Utility / Global / UI-Part / Module(拡張コンポーネント)」の4階層に再構築します。
- 拡張コンポーネントの順序: レイアウト→特定パーツの順に整理します
- スクロールの一括定義: 頻出するスクロールエリアに対し、Utility Class（.scroll-y）を作成し、scrollbar-width などの設定をDRY（Don't Repeat Yourself）にします。
- モダン化による省コード化: calc(var(--unit) * N) を維持しつつ、gap や flex のレイアウト指定を活用て子要素のレイアウト記述を削減します
