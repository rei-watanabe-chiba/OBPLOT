...
## 3. 次の開発作業
- Tab2のPreviw層開発と設計書の更新と機能実装を平行して行う。
- コード作成時はコード量の節約を考慮して、保守性と設計思想を保ちつつ統合・共通化、高級言語の仕様とモダンな記述方法を採用する。
- ただし、深すぎるネストは避けて、全体のトーンを統一する。
- 現在はmakePreviewを仮実装し、デバッグで補正ロジックの動作を確認。
- Previw層実装にともなって追加した各htmlのコードのリファクタリングを進める
