...
- 現状のCDN依存の方式からCDN障害も破壊的変更も無縁であり、**10年後も動く最も堅牢な方法**への転換を図る

## 4. UIのモダン化のアプローチ
- **段階的アプローチ**: 一気に全改修するのではなく、セレクタ依存部（Event/View層）を完全に保護しながら、局所的なコンポーネント・CSSスタイルから段階的にトーンを確認する。
- **ブランドカラーの完全維持**: 既存のカラーパレットをCSS変数として堅持し、モダンUIのデザインに調和させる。
- **サイドバーUIへの準拠**: 300pxのサイドバー内においてUXを意識して文字サイズや間隔を整える。ただし、保守性paddingとmarginの競合や多重設定は回避し、シンプルなCSS記述を心がける
