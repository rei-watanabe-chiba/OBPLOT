...
## 6. 次の開発手順
- 要件1. DataUtils.createStatsRows⁠ を改修し、デフォルトで生データのペア（ラベル・値）を返す仕様に変更。引数で小数点桁数や文字列整形モードを指定可能とし、具体的な表示ラベルはユーザーとの対話を通じて決定する設計へリファクタリングする。
- 要件2. グラフ描画の連動state変更が、ボタン駆動の静的発火と一括 State 更新であることを前提に、Chart.html 側の過剰な防衛的コードを最小限にスリム化し、単方向データフローによる描画パイプラインのシンプル化を推進する。
- 要件3. 要件2と同時並行でグラフ描画ライブラリのApache ECharts
への変更を行い、既存のSVG描画の標準機能への移行、保守性の向上、コード量の節約を行う。
