...
変数名・関数名の命名規則統一およびコード量削減（スリム化）を極限まで高めるリファクタリングを開始します。
**
**重要な絶対的鉄則**:「Single Source of Truth（state駆動）に基づく単方向データフローと状態の不可侵性。UIを直接の正とせず、Stateの変更のみがアプリを駆動する原則をいかなるアプローチでも絶対に死守してください。

以下の4つのステップを「Pro版の深い思考（CoT）」を用いて、順を追って網羅的に実行してください。
...
### Step 1: コード全体の厳密な精査
- プロジェクト全ファイルの構造と、タブ・セクション依存のステート管理（`Tab1ST`, `Tab2ST`）およびフェーズ管理の依存関係を完全に把握してください。
- モジュール間で共通参照される「A（グローバル・パブリック）」と、関数内でのみ使用される「B（ローカル・プライベート）」の識別子を分類・リストアップしてください。

### Step 2: パターン抽出と整理
- 冗長な記述、重複しているボイラープレート、手動バインディングの箇所を特定し、ボトルネックとなっている構造を抽出してください。

### Step 3: 競合・予約語・回避事項の確認
- JavaScript / GAS (V8) の予約語、DOM標準プロパティ、既存のグローバル名前空間（`State`, `Ctrl`, `Mtd`, `DOM`, `Tpl` 等）との競合リスクがないかを完全にチェックしてください。

### Step 4: 論理構成の改変強度と圧縮度に基づくアプローチの構築
「state駆動」の設計思想を厳守することを大前提として、以下の軸（論理構成の改変強度）と、競合・予約語を回避した上での短縮レベル（3段階の圧縮度）を組み合わせた複数のリファクタリング案を構築・提案してください。

1. **【安定型】現在の処理フローを維持するパターン**
   - 既存のメソッドチェーン、フェーズ遷移、タブ・セクション構造のフローをそのまま保ちつつ、モジュール内のローカル変数や記述を最適化するアプローチ。
2. **【転換型】処理フローの構造改革を許可するパターン**
   - state駆動の理念はそのままに、冗長なDOM操作や手動DIマップ登録、セクション依存の重複処理を動的解決や自動バインディング等で抜本的に刷新するアプローチ。

- **圧縮度（3段階の短縮レベル）**:
  - **Level 1 (標準記述)**: 命名の可読性を重視し、冗長な修飾語のみを排除したバランス型。
  - **Level 2 (コンパクト)**: スコープの狭さを活かし、標準的な省略形（`row` $\rightarrow$ `r` 等）を積極的に取り入れたスリム型。
  - **Level 3 (極限短縮)**: 予約語や競合に細心の注意を払いつつ、1〜2文字の変数名やパイプライン処理の極小化を狙った高密度型。

---
上記4ステップの分析結果と具体的な設計・コード案を、まずは **【Step 1〜3の精査・確認結果】** として整理して提示してください。その後、あなたの提案する設計案に基づき、対話を進めていきましょう。
