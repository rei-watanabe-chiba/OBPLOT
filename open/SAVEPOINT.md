...
## 7. 開発状況と次ステップ
- **現在の状況**: tab2レポート出力の実装
- GAS版、エクセル版で共通するコードで描画
- 別タブ（エクセル版はダウンロード式）で表示
- repo層に渡すデータは、tab2内でのデータstateの状況を整理してすりあわせる
- repo層はプレビュー層解放と同時にアクティブにするが、自動オープンはしない。
- repo層には改めてitem選択マルチボックを追加、適用する補正値セットを選択する標準セレクトを追加（選択肢は、新規検量線、＋correction内のIndexName）
- itemフィルターされたデータと検量線データを別タブプレビューに注入
- 別タブプレビューでは、左に操作サイドバー、右の描画領域にA4比率で最大９枚のグラフを配置する
- プレビュー内サイドバーではシンボルの変更、defaultGraphStyle、各グラフに表示する元素・指数を変更するUIを配置する
- まずはこのrepo層の設計書をすり合わせる。
- また既存の変数管理にdefaultGraphStyleをを初期値として読み取り、動的にグラフスタイルstateを生成する機構を組み込む
