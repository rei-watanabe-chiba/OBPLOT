...
## 7. 開発状況と次ステップ
- **現在の状況**: 機能開発は止めてUIの微調整
- #restore(bu)をTab2でのみ有効にする（tab1ではストレージがあっても無視）
- class IdlTmで時間経過自、ダイアログの押下も不可能となるため、ダイアログで終了と自動状態復元をやめる
- アイドル経過自はOKキャンセルのないダイアログ（dialog id="idleToastを汎用化（タイトルメッセージを指定可能にする）する
- コンポーネントにトースト関数追加してイベントから呼び出し。
- タイトルは「GAS通信エラー」メッセージは「一定時間操作がなかったため、サイドバーを再起動してください」
- 再起動後は通常通りの復元確認ダイアログ。つまりauto判定機能は消失させる。
