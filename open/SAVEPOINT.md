...
## 7. 開発状況と次ステップ
- **現在の状況**: GAS / Web版 Excel Add-in の両環境における結合SPAの稼働テストと、環境差異の動的吸収（Adapter）が正常に完了。
- **直近の課題と解決策（完了）**:
  - Web版Excel特有の iframe サンドボックスによる `<dialog>` 表示エラーに対するCSSフォールバック（`.dialog-fallback`）を実装。
  - `infra/build.js` によるExcelビルド時限定の `office.js` 自動インジェクション機構を確立。
  - 過剰防衛ロジック（起動時の強制タイマー等）や不要なHTMLエスケープ処理のデッドコードを排除し、コードベースをクリーンアップ。
  - `GLB.idx` による名前空間の絶対定義と、`Mtd.Util.buildHeader` への動的ヘッダー生成の集約を完了。ヘッダーを厳密に定義し曖昧なプロパティ結合によるデータロスト問題を根本解決。
  - プレビュー時の空欄・無効値に対する「行除外」を廃止し、回帰計算実行の直前でのみ安全に数値ペアを抽出する遅延防衛ロジックを実装。
- **次ステップ（最優先タスク）**:
  - **過剰防衛・デッドコードの整理**: 度重なる環境結合SPAの動作確認やエラー回避の試行錯誤で追加された一時的なフォールバック、重複コード、不要になった例外処理などをさらに洗い出し、ECS内容の見直しと改修・動作テストを行い、コードベース全体のリファクタリング（クリーンアップ）を優先する。機能拡張は未定。
