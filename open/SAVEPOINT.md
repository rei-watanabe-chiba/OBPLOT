...
- **直近の課題と解決策（完了）**:
  - `Report.html` 結合時におけるHTMLパーサー誤認エラーに対するエスケープ処理を実装。
  - Web版Excel特有の iframe サンドボックスによる `<dialog>` 表示エラーに対するCSSフォールバックを実装。
  - Microsoft提供アイコンのリンク切れを防ぐ自作アイコンホスティングと、マニフェストのキャッシュバスティック（`&v=2`）を適用。
  - `infra/build.js` によるExcelビルド時限定の `office.js` 自動インジェクション機構を確立し、環境誤認（`google is not defined`）を完全解消。
- **次ステップ（最優先タスク）**:
  - **過剰防衛・デッドコードの整理**: 度重なる動作確認やエラー回避の試行錯誤で追加された一時的なフォールバック、重複コード、不要になった例外処理などを洗い出し、ECS内容の見直しと改修・動作テストを行い、コードベース全体のリファクタリング（クリーンアップ）を最初に行う。その上で本格的な機能拡張へ移行する。
